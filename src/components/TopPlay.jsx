      ...
      <div className="w-full flex flex-col mt-8">
        ...
        >
          {topPlays?.slice(0, 5).map((artist) => (
            ...
            >
            <Link to={`/artists/${artist?.relationships?.artists?.data[0]?.id}`}>
                <img src={artist?.attributes?.artwork?.url} alt={artist?.attributes?.name} className="rounded-full w-full object-cover" />
            </Link>
            </SwiperSlide>
          ))}
        </Swiper>
